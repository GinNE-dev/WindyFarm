...
namespace WindyFarm.Gin.Game.Shop
{
    public class Bakery
    {
        public readonly List<CakeShop> ShopData;
        public readonly List<ItemSellPrice> ItemSellPrices;
        public float PriceFactor { get; private set; } = 1f;
        public Bakery(List<CakeShop> farmingShopData, List<ItemSellPrice> itemSellPrices)
        {
        ...
        public List<CakeShop> GetSellData() => [.. ShopData.OrderBy(d => d.SlotIndex)];
        ...
        {
            Message? m = MessagePool.Instance.Get(MessageTag.BakeryResponse);

            if (m is null || m is not BakeryDataMessage) return null;

            var data = (BakeryDataMessage)m;
            data.ItemIds = new();
